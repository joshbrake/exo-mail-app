...
// ---------------------------------------------------------------------------
// API helpers — uses generic extensions:invoke IPC so this extension works
// as bundled, private, or installed without preload modifications.
// ---------------------------------------------------------------------------
...
/**
 * Invoke an Asana extension IPC method via the generic extensions:invoke channel.
 * Unwraps the { success, data, error } envelope.
 */
async function asanaInvoke<T>(method: string, params?: unknown): Promise<T> {
  const result = (await getExtensionsApi().invoke("asana", method, params)) as {
    success: boolean;
    data?: T;
    error?: string;
  };
  if (!result.success) {
    throw new Error(result.error || `Asana IPC call failed: ${method}`);
  }
...
function TokenInput({ onSaved }: { onSaved: () => void }) {
  ...
  const handleSave = useCallback(async () => {
    ...
    setError(null);
    try {
      await asanaInvoke("save-token", { token: token.trim() });
      onSaved();
    } catch (err) {
...
export function AsanaTaskPanel({ email, threadEmails }: AsanaTaskPanelProps): React.ReactElement {
  ...
  useEffect(() => {
    asanaInvoke<{ authenticated: boolean }>("check-auth")
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      asanaInvoke<{ workspaces: AsanaWorkspace[] }>("get-workspaces"),
    ])
      .then(([wsResult, projResult, { workspaces: ws }]) => {
        ...
        const wsToSelect =
          savedWorkspace && ws.some((w) => w.gid === savedWorkspace)
            ? savedWorkspace
            : ws.length === 1
              ? ws[0].gid
              : "";
  ...
  useEffect(() => {
    ...
    Promise.all([
      asanaInvoke<{ taskList: { gid: string; name: string } }>("get-user-task-list", {
        workspaceGid: selectedWorkspace,
      }),
      asanaInvoke<{ projects: AsanaProject[] }>("get-projects", {
        workspaceGid: selectedWorkspace,
      }),
    ])
  ...
  const saveDrafts = useCallback(
    (cards: TaskCardState[]) => {
      ...
      asanaInvoke("save-draft-tasks", { threadId: email.threadId, drafts: pending });
    },
    [email.threadId],
  );
  ...
  useEffect(() => {
    ...
    asanaInvoke<{ tasks: LinkedTaskInfo[] }>("get-linked-tasks", { threadId: email.threadId })
      .then(({ tasks }) => setLinkedTasks(tasks))
      .catch(() => setLinkedTasks([]))
      .finally(() => setLinkedTasksLoading(false));

    // Load persisted draft tasks
    asanaInvoke<{ drafts: Array<{ title: string; description: string; dueDate: string }> }>(
      "get-draft-tasks",
          ...
          setTaskCards(
            drafts.map((d) => ({
              task: {
                ...
                suggestedDueDate: d.dueDate || null,
              },
  ...
  const handleAnalyze = useCallback(async () => {
    ...
    try {
      ...
      const result = await asanaInvoke<{ tasks: SuggestedTask[] }>("suggest-tasks", {
        emailId: email.id,
  ...
  const handleAddToAsana = useCallback(
    async (index: number) => {
      ...
      try {
        ...
        const result = await asanaInvoke<{
          success: boolean;
          task: { gid: string; permalink_url: string };
        }>("create-task", {
  ...
  const handleAddBlankTask = useCallback(() => {
    const blank: TaskCardState = {
      task: { title: "", description: "", suggestedDueDate: null },
      status: "pending",
      editedTitle: "",
      editedDescription: "",
      editedDueDate: "",
  ...
  return (
    <div className="p-3 space-y-3 overflow-hidden">
      ...
      {authenticated === false && taskCards.length > 0 && (
        <TokenInput
          onSaved={() => {
      ...
      {/* Workspace/project selectors — show when authenticated and tasks exist */}
      {authenticated && taskCards.length > 0 && (
        <div className="space-y-1.5">
          <select
